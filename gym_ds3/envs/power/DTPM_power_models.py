...
# def decrease_frequency(args, resource, current_PE, timestamp):
#     for OPP_i, OPP_tuple in enumerate(resource.OPP):
#         if OPP_tuple[0] == current_PE.current_frequency:
#             if OPP_i == 0:
#                 # Already at min freq
#                 current_PE.current_frequency = resource.OPP[OPP_i][0]
#                 current_PE.current_voltage = resource.OPP[OPP_i][1]
#                 if (args.DEBUG_SIM):
#                     print('[D] Time %d: PE %s - The frequency is already at the minimum: %d' % (timestamp, current_PE.name, current_PE.current_frequency))
#                 for task in current_PE.queue:
#                     task.min_freq = current_PE.current_frequency
#                     task.sum_freq.append(current_PE.current_frequency)
#                     task.avg_freq+=current_PE.current_frequency
#                 return False
#             else:
#                 # Decrease the frequency to the previous OPP
#                 current_PE.current_frequency = resource.OPP[OPP_i - 1][0]
#                 current_PE.current_voltage = resource.OPP[OPP_i - 1][1]
#                 if (args.DEBUG_SIM):
#                     print('[D] Time %d: PE %s - The frequency was decreased: %d' % (timestamp, current_PE.name, current_PE.current_frequency))
#                 #scadd
#                 for task in current_PE.queue:
#                     if(task.min_freq>current_PE.current_frequency):
#                        task.min_freq = current_PE.current_frequency
#                     if(task.max_freq<current_PE.current_frequency):
#                        task.max_freq = current_PE.current_frequency
#                     task.sum_freq.append(current_PE.current_frequency)
#                     task.avg_freq+=current_PE.current_frequency
#                 #scadd
#                 return True

# def increase_frequency(resource, current_PE, timestamp):
#     for OPP_i, OPP_tuple in enumerate(resource.OPP):
#         if OPP_tuple[0] == current_PE.current_frequency:
#             if OPP_i == len(resource.OPP) - 1:
#                 # Already at max freq
#                 current_PE.current_frequency = resource.OPP[OPP_i][0]
#                 current_PE.current_voltage = resource.OPP[OPP_i][1]
#                 if (args.DEBUG_SIM):
#                     print('[D] Time %d: PE %s - The frequency is already at the maximum: %d' % (timestamp, current_PE.name, current_PE.current_frequency))
#                 for task in current_PE.queue:
#                     task.max_freq = current_PE.current_frequency
#                     task.sum_freq.append(current_PE.current_frequency)
#                     task.avg_freq+=current_PE.current_frequency
...
#                 current_PE.current_voltage = resource.OPP[OPP_i + 1][1]
#                 if (args.DEBUG_SIM):
#                     print('[D] Time %d: PE %s - The frequency was increased: %d' % (timestamp, current_PE.name, current_PE.current_frequency))
#                 #scadd
#                 for task in current_PE.queue:
#                     if(task.min_freq>current_PE.current_frequency):
#                        task.min_freq = current_PE.current_frequency
...
# def set_max_frequency(args, resource, current_PE, timestamp):
#     max_freq = get_max_freq(resource.OPP)
#     max_voltage = get_max_voltage(resource.OPP)
#     current_PE.current_frequency = max_freq
#     current_PE.current_voltage = max_voltage
#     if (args.DEBUG_SIM):
#         print('[D] Time %d: PE %s - The frequency was set to the maximum: %d' % (timestamp, current_PE.name, current_PE.current_frequency))
#     #scadd
#     for task in current_PE.queue:
#         if(task.min_freq>current_PE.current_frequency):
#            task.min_freq = current_PE.current_frequency
...
# def keep_frequency(resource, current_PE, timestamp):
#     current_PE.current_frequency = current_PE.current_frequency
#     current_PE.current_voltage = current_PE.current_voltage

#     if (args.DEBUG_SIM):
#         print('[D] Time %d: PE %s - The frequency was not modified: %d' % (timestamp, current_PE.name, current_PE.current_frequency))
#     for task in current_PE.queue:
#         task.sum_freq.append(current_PE.current_frequency)
#         task.avg_freq+=current_PE.current_frequency

# def set_L2_best_frequency(resource, current_PE, timestamp, L2_best_frequency):
#     current_PE.current_frequency = L2_best_frequency
#     current_PE.current_voltage = get_voltage_constant_mode(resource.OPP,L2_best_frequency)
#     if (args.DEBUG_SIM):
#         print('[D] Time %d: PE %s - initial task frequency was set to : %d' % (timestamp, current_PE.name, current_PE.current_frequency))
